<?php  if(isset($_POST['com']))  {   if($_POST['com'] == 'del')   {    foreach($_POST as $key => $value)    {     $value = ValidNumber($value,true);     if(NoNumbers($key) == 'cb')      $pm->SetFlag(RECIVER,$value, DEL_RECIVER);    }   }   elseif($_POST['com'] == 'arc')   {    foreach($_POST as $key => $value)    {     $value = ValidNumber($value,true);     if(NoNumbers($key) == 'cb')      $pm->RemoveFlag(RECIVER, $value, ARC_RECIVER);    }   }  }  $count = $pm->Counts(RECIVER, ARC_RECIVER, DEL_RECIVER);  $page = isset($_GET['page']) ? (int)$_GET['page'] : 0;  $row = isset($_GET['row']) ? (int)$_GET['row'] : 10;  if($row >100)   $row = 100;  if(!$plus->HavePlus('pb'))   $row = 10;    if($page * $row > $count)   $page = 0;    $list = $pm->GetList(RECIVER, ARC_RECIVER, DEL_RECIVER,$page,$row);  ?>
<div class="massageList">
<form action="pm.php?show=<?php echo ARC_IN;?>&amp;page=<?php echo $page;?>&amp;row=<?php echo $row?>" method="post" name="msg" id = "msg">
<input type="hidden" name="com" id="com" value="?" />
<table class="hovertable">
<tr>
<td colspan="5" class="center"><?php echo $lang['َArcInbox'];?></td>
</tr>
<tr>
<td style="width:16px;text-align:center;">&nbsp;</td>
<td style="width:16px;text-align:center;">&nbsp;</td>
<td style="width:300px;text-align:center;"><span class=""><?php echo $lang['Subject'];?></span></td>
<td style="width:125px;text-align:center;"><span class=""><?php echo $lang['Sender'];?></span></td>
...
<input type="button" onclick="SubmitForm('msg','com','arc');" class="buttonClass" value = "<?php echo $lang['Box'];?>"/>
</td></tr>
<tr>
<td colspan="2"><?php echo $lang['PageNumber'];?></td>
<td colspan="3">
<div class="pagination">
<?php echo Paging($count,sprintf('pm.php?show=%s&amp;',ARC_IN), $row, $page);?>
</div>
</td>
</tr>
</table>
...
<input type="hidden" name = "show" value="<?php echo ARC_IN;?>"/>
