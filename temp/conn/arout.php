<?php  if(isset($_POST['com']))  {   if($_POST['com'] == 'del')   {    foreach($_POST as $key => $value)    {     $value = ValidNumber($value,true);     if(NoNumbers($key) == 'cb')      $pm->SetFlag(SENDER,$value, DEL_SENDER);    }   }   elseif($_POST['com'] == 'arc')   {    foreach($_POST as $key => $value)    {     $value = ValidNumber($value,true);     if(NoNumbers($key) == 'cb')      $pm->RemoveFlag(SENDER, $value, ARC_SENDER);    }   }  }  $count = $pm->Counts(SENDER, ARC_SENDER, DEL_SENDER);  $page = isset($_GET['page']) ? (int)$_GET['page'] : 0;  $row = isset($_GET['row']) ? (int)$_GET['row'] : 10;  if($row >100)   $row = 100;  if(!$plus->HavePlus('pb'))   $row = 10;    if($page * $row > $count)   $page = 0;    $list = $pm->GetList(SENDER, ARC_SENDER, DEL_SENDER,$page,$row);  ?>
<div class="massageList">
<form action="pm.php?show=<?php echo ARC_OUT;?>&amp;page=<?php echo $page;?>&amp;row=<?php echo $row?>" method="post" name="msg" id = "msg">
<input type="hidden" name="com" id="com" value="?" />
<table class="hovertable">
<tr>
<td colspan="5" class="center"><?php echo $lang['َArcoutbox'];?></td>
</tr>
<tr>
...
<?php echo Paging($count,sprintf('pm.php?show=%s&amp;',ARC_OUT), $row, $page);?>
</div>
</td>
</tr>
</table>
...
<input type="hidden" name = "show" value="<?php echo ARC_OUT;?>"/>
