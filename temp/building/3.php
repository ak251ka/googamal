<div class="b_troop">
<?php  define('CHANGE_WORKER','cwor');  if(isset($_POST['com']) and $_POST['com'] == CHANGE_WORKER)  {   if(isset($_POST['kind']) and isset($_POST['subkind']))   {    $_POST['subkind'] = ValidNumber($_POST['subkind'],true);    $freeW = $town->workers - ($town->w1 +$town->w2 +$town->w3 +$town->w4 +$town->w5);    if($town->Lock())    {    if($_POST['kind'] == 'w3')    {     if($freeW + $town->w3 >= $_POST['subkind'])     {      $town->w3 = $_POST['subkind'];      $town->SetField(array('w3' => $town->w3));     }     $town->SetProduct();    }    }    $town->UnLock();   }  }  $pro = $dbo->ExectueScaler(   sprintf('SELECT SUM(`func`) AS `func` FROM `%sbuilding_d` WHERE `bid` =\'%s\' AND `lvl` <= \'%s\'',DB_PERFIX,$town->$b, $town->$u + 1),'func');  printf($lang['LevelPoint'].'<br />',$town->p3);  if($town->$u != $info['mb'])   printf($lang['NextPoint'],$pro);  if($town->GetLevel(28) == -1){   echo '<center>'.$lang['NoRequirement'].'</center>';   printf('<center><b>%s %s</b></center>',$lang['Requirement'],$lang['b28']);   return;  }    $freeWorker = $town->workers -($town->w1 + $town->w2 + $town->w3 + $town->w4 + $town->w5);  $pb = $plus->HavePlus('r3')? PLUS_RESOURCE_POINT: 1;  $a = $pb * $town->p3 * WORKER_POINT + $town->p3 * WORKER_POINT * $town->pclo/100;  $b = $pb * $town->p3;  ?>
...
<input type="hidden" name = "kind" value="w3" />
<input type="hidden" name="subkind" id = "subkind" value="<?php echo $town->w3;?>"/>
<table class="t200">
<tr><td colspan="2"><?php printf('<center><b>%s</b></center>',$lang['Worker']);?> </td></tr>
<tr><td><?php echo $lang['Product'];?></td><td width="50px" id = "producted"><?php echo ceil($town->w3 * $a + $b); ?></td></tr>
...
values:{min:0,max:<?php echo $town->w3 + $freeWorker;?>,mid:<?php echo $town->w3;?>,fontSize:"10px"},fire:setWorker})
</script>